...
export default function remarkSplitSections(options = {}) {
  ...
  return async function transformer(tree, file) {
    ...
    file.data.fm.totalReadingTime = sections.reduce((total, section) => total + section.readingTime, 0)

    // Log splitting information in development
    if (process.env.NODE_ENV === 'development') {
      console.log(`📚 Split large post "${file.data.fm.title || 'Unknown'}" into ${sections.length} sections`)
