...
`npm run dev`나 `docker compose up` 같은 명령어를 실행해 뒀는데, 카페 와이파이가 잠깐 끊겨서 SSH 접속이 종료되는 경우가 있다.
...
**팁:** 실전에서는 아래 명령어를 제일 많이 쓴다. **"없으면 만들고, 있으면 들어가라"**는 뜻이다.
...
홈 디렉터리에 `.tmux.conf` 파일을 만들고 아래 내용을 붙여넣으면 된다. (`vim ~/.tmux.conf`)
...
- **`set -g mouse on`**: 마우스로 화면 분할 경계선을 잡고 드래그하거나, 휠로 스크롤할 수 있다. 처음 쓸 때 체감이 가장 컸던 설정이다.
