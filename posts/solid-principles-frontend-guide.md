...
updated: '2026-06-12T16:54:35+09:00'
tags:
  - frontend
...
SOLID는 객체지향 프로그래밍의 5가지 설계 원칙이다. 하지만 이 원칙들은 React나 Vue 같은 컴포넌트 기반 개발에서도 놀라울 정도로 잘 들어맞는다. 복잡해지는 프론트엔드 코드를 정리할 때 나에게는 이 원칙들이 좋은 나침반이 됐다.
...
좋은 코드는 하루아침에 만들어지지 않는다. 하지만 SOLID 원칙을 나침반 삼아 조금씩 개선해나가다 보면, 어느새 과거의 나보다 나은 코드를 쓰고 있는 나를 발견하게 된다.

다음에 "이 컴포넌트 좀 수정해주세요"라는 요청이 들어왔을 때, 당황하는 대신 "어, 이 부분은 깔끔하게 분리되어 있네"라고 생각할 수 있기를. 그런 날을 위해 오늘도 컴포넌트 하나를 쪼갠다.
