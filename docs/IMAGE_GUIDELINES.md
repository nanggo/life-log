...
로컬 이미지는 빌드 시점에 원본과 함께 반응형 WebP 변형본과 크기 메타데이터가 생성됩니다.

- ✅ 본문 폭 기준 672px WebP와 원본을 넘지 않는 최대폭 WebP 자동 생성
- ✅ 크기 정보 자동 추출
- ✅ CLS 완전 방지
...
브라우저는 화면 폭과 DPR에 맞는 변형본을 선택하고, 이미지 확대 모달은 원본을 사용합니다.

정적 이미지는 브라우저와 CDN에 장기 캐시됩니다. 이미 게시된 이미지를 교체할 때는 같은
경로를 덮어쓰지 말고 `cover-v2.webp`처럼 파일명을 바꾼 뒤 frontmatter나 본문 경로를 함께
수정합니다.
...
- 🚀 **로컬 이미지**: 빌드 타임 반응형 WebP 생성 + 0 CLS
